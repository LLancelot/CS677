'''
implement Gaussian SVM classifier
'''
...
svm_classifier_Gaus = svm.SVC(kernel='rbf')
svm_classifier_Gaus.fit(year1_re_and_vo,year1_Label)
predicted_Gaus = svm_classifier_Gaus.predict(testing_data)
accuracy_Gaus = round(sum(predicted_Gaus==data2['Label'].values.ravel())/len(data2['Label'].values.ravel())*100,2)
...
print("Accuracy of Gaussian SVM classifier for year 2:", str(accuracy_Gaus)+"%")

if accuracy_Gaus > accuracy_linear:
    print("The accuracy using Gaussian SVM is better than Linear SVM.")
else:
    print("The accuracy using Gaussian SVM is worse than Linear SVM.")
