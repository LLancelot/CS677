...
class Custom_KNN():
    ...
    def predict(self, new_x:pd.DataFrame):
        ...
        matrix = confusion_matrix(y_true=np.asarray(year2_Label), y_pred=self.test_predict_Label)
        print("P=",self.distance_parameter_p)
        print("The recall =", round(matrix[0][0] / sum(matrix[0]),2))
        print("The true negative rate =", round(matrix[1][1] / sum(matrix[1]),2))
...
print("In year2, for p=1.5 and p=2, the recall and true negative rate are the same,"
      "\nfor p=1, the recall is the same but the true negative rate is different.")